...
describe('期待した値を保持したモデルを生成する', () => {
  ...
  describe('重複する要素が存在する値で生成した場合', () => {
    ...
    const ids = new EntryPlayers(values);
...
describe('contains メソッド', () => {
  const players = new EntryPlayers([new PlayerId('0'), new PlayerId('1')]);
...
describe('change メソッド', () => {
  describe('入れ替え対象の要素を保持している場合', () => {
    const players = new EntryPlayers([new PlayerId('0'), new PlayerId('1')]);
    const from = new PlayerId('1');
    const to = new PlayerId('2');

    const newPlayers = players.change(from, to);

    test('入れ替え対象の要素を入れ替えた新しいオブジェクトを返す', () => {
      const actual = Array.from(newPlayers);
      const expected = [new PlayerId('0'), new PlayerId('2')];
  ...
  describe('入れ替え対象の要素を保持していない場合', () => {
    const players = new EntryPlayers([new PlayerId('0'), new PlayerId('1')]);
    const from = new PlayerId('unknown');
    const to = new PlayerId('2');
